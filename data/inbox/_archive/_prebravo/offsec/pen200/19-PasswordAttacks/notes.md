...
- leveraging password hashses
  ...
  - Linux
    - /etc/shadow![Alt text](_archive/_prebravo/offsec/pen200/19-PasswordAttacks/image.png)
    ...
    - windows
      ...
      - mimikatz facilitates password hash extraction from the Local Security Authority Subsystem (LSASS) process memory where they are cached
        ...
        - ```shell mimikatz # lsadump::sam```     dump the contents of the same database        ![Alt text](_archive/_prebravo/offsec/pen200/19-PasswordAttacks/image-1.png)
      ...
      - The Pass-the-Hash (PtH) technique (discovered in 1997) allows an attacker to authenticate to a remote target by using a valid combination of username and NTLM/LM hash rather than a clear text password. This is possible because NTLM/LM password hashes are not salted and remain static between sessions. Moreover, if we discover a password hash on one target, we cannot only use it to authenticate to that target, we can use it to authenticate to another target as well, as long as that target has an account with the same username and password.![Alt text](_archive/_prebravo/offsec/pen200/19-PasswordAttacks/image-4.png)
